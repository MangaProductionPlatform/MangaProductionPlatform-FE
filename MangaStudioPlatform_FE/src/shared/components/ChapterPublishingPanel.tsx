...
export function ChapterPublishingPanel() {
  ...
  const [readyChapters, setReadyChapters] = useState<
    ReadyForPublishChapterDto[]
  >([]);
  ...
  const selectedSchedule = useMemo(
    () =>
      scheduledChapters.find((chapter) => chapter.chapterId === chapterId) ??
      null,
    [chapterId, scheduledChapters],
  );
  const isSelectedScheduleLocked = Boolean(
    selectedSchedule && editingScheduleId !== selectedSchedule.chapterId,
  );
  ...
  const loadReadyChapters = async (showToast = false, showLoading = true) => {
    ...
    try {
      const [items, scheduled] = await Promise.all([
        mangaErpApi.getReadyForPublish(),
        mangaErpApi.getPublishingSchedule(),
      ]);
      setReadyChapters(items);
      setScheduledChapters(scheduled);
      if (showToast)
        toast.success(
          "Ready chapters refreshed",
          `${items.length} chapter(s) returned.`,
        );
    } catch (error) {
      toast.error(
        "Could not load ready chapters",
  ...
  const selectChapter = (id: string) => {
    ...
    setIssueType(
      isPublicationType(chapter.issueType) ? chapter.issueType : "Weekly",
    );
  ...
  const schedule = async () => {
    ...
    if (!trimmedChapterId || !trimmedSeriesId || !publishDate) {
      toast.error(
        ...
        "Select a chapter, confirm its series, and choose a future publish time.",
      );
      return;
    }
    if (isSelectedScheduleLocked) {
      toast.error(
        "Schedule already exists",
        "Select Edit schedule for this chapter before making changes.",
      );
      return;
    }
    ...
    try {
      ...
      setLastResult(
        `${editingScheduleId ? "Updated" : "Scheduled"} ${issueType} publication for ${new Date(isoPublishDate).toLocaleString()}.`,
      );
      toast.success(
        editingScheduleId
          ? "Publication schedule updated"
          : "Publication scheduled",
        `${issueType} at ${new Date(isoPublishDate).toLocaleString()}`,
      );
      setEditingScheduleId("");
  ...
  const cancelSchedule = async (chapter: PublishingScheduleItemDto) => {
    if (!window.confirm(`Cancel publication schedule for ${chapter.title}?`))
      return;
    try {
  ...
  const publishNow = async () => {
    ...
    if (!trimmedChapterId) {
      toast.error(
        "Chapter is required",
        "Select an approved chapter before publishing.",
      );
      return;
    }
    ...
    try {
      ...
      setLastResult(
        `Published at ${new Date(result.publishedAt).toLocaleString()}. ${result.publicationUrl}`,
      );
      toast.success(
        "Chapter published",
        result.publicationUrl || result.status,
      );
      await loadReadyChapters(false, false);
    } catch (error) {
      toast.error(
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-200">
              Editorial Board publishing
            </p>
            <h2 className="mt-2 text-xl font-bold text-white">
              Chapter release control
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
              Select an approved chapter, choose Weekly, Monthly, or Special
              release, then schedule automated publishing or publish
              immediately.
            </p>
          </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            {readyChapters.length} chapter(s) ready or scheduled
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
            ...
            {readyChapters.map((chapter) => (
              <option key={chapter.chapterId} value={chapter.chapterId}>
                ...
                {chapter.scheduledPublishAt
                  ? ` (scheduled ${new Date(chapter.scheduledPublishAt).toLocaleString()})`
                  : ""}
              </option>
            ))}
          </select>
        ...
        <label className="text-sm text-slate-400">
          ...
          <select
            ...
            value={issueType}
            onChange={(event) =>
              setIssueType(event.target.value as PublicationType)
            }
          >
            {issueTypes.map((type) => (
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
        ...
        <div className="rounded-xl border border-white/10 bg-slate-950 p-4">
          ...
          {isSelectedScheduleLocked ? (
            <p className="mt-2 text-xs font-semibold text-amber-200">
              A publication is already scheduled. Use Edit schedule below to
              update it.
            </p>
          ) : null}
        </div>
        ...
        <div className="mt-4 space-y-3">
          {scheduledChapters.map((chapter) => (
            <article
              ...
              className="flex flex-wrap items-center justify-between gap-3 border border-white/10 bg-slate-950 p-4"
            >
              <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() => void cancelSchedule(chapter)}
          ...
          {!isLoading && !scheduledChapters.length ? (
