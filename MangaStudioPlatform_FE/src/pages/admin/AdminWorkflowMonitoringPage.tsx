...
export default function AdminWorkflowMonitoringPage() {
  ...
  const loadStats = async (showLoading = true) => {
    ...
    try {
      ...
      toast.error("Could not load workflow stats", error instanceof Error ? error.message : "Unknown error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    async function loadInitialStats() {
      try {
        ...
        if (!ignore) toast.error("Could not load workflow stats", error instanceof Error ? error.message : "Unknown error");
      } finally {
  ...
  const totals = useMemo(() => ({
    submissions: stats?.submissionStats.reduce((sum, item) => sum + item.count, 0) ?? 0,
    chapters: stats?.chapterStats.reduce((sum, item) => sum + item.count, 0) ?? 0,
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-black text-white">Workflow monitoring</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
            Backend status breakdown across submissions, chapters, and page tasks.
          </p>
        </div>
        <button type="button" onClick={() => void loadStats()} disabled={isLoading} className="btn-secondary inline-flex items-center gap-2">
      ...
      {isLoading ? <p className="rounded-lg border border-white/10 bg-slate-900/75 p-5 text-sm text-slate-300">Loading workflow stats...</p> : null}

      {stats ? (
        <section className="grid gap-5 xl:grid-cols-3">
          <StatusColumn title="Submission statuses" items={stats.submissionStats} />
...
function StatusColumn({ title, items }: { title: string; items: Array<{ status: string; count: number }> }) {
  return (
    <section className="rounded-lg border border-white/10 bg-slate-900/75 p-5">
      <h3 className="font-bold text-white">{title}</h3>
      <div className="mt-4 space-y-3">
        {items.map((item) => (
          <div key={item.status} className="flex items-center justify-between gap-3 rounded-lg bg-slate-950 px-3 py-2 text-sm">
