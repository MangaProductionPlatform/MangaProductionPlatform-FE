...
import { CheckCircle2, Eye, MessageSquare, RefreshCw, Send, XCircle } from "lucide-react";
import { mangaErpApi } from "../../shared/services/mangaErpService";
import type { SubmissionDetailDto, SubmissionSummaryDto } from "../../shared/types/mangaErp";
import { useToast } from "../../shared/components/toastContext";

type EditorAction = "start" | "recommend" | "revision" | "reject";
...
export default function ReviewQueuePage() {
  ...
  const [runningAction, setRunningAction] = useState<EditorAction | null>(null);
  ...
  const openSubmission = async (id: string) => {
    try {
      ...
      setSelected(detail);
      setFeedback(detail.feedbackMessage ?? "");
    } catch (err) {
      toast.error(
  ...
  const runAction = async (action: EditorAction) => {
    if (!selected) {
      toast.error("Select a submission", "Open one queue item before sending an action.");
      return;
    }

    if ((action === "recommend" || action === "revision" || action === "reject") && !feedback.trim()) {
      toast.error("Feedback required", "Enter a note before sending this action.");
      return;
    }
    ...
    try {
      if (action === "start") {
        await mangaErpApi.startSubmissionReview(selected.id);
        toast.success("Review started", "This submission is assigned to you.");
      }

      if (action === "recommend") {
        await mangaErpApi.recommendSubmission(selected.id, {
          recommendationMessage: feedback.trim(),
        });
        toast.success("Recommended", "The submission was sent to Editorial Board.");
      }

      if (action === "revision") {
        await mangaErpApi.teRequestSubmissionRevision(selected.id, {
      ...
      if (action === "reject") {
        await mangaErpApi.teRejectSubmission(selected.id, {
          reason: feedback.trim(),
        });
        toast.success("Rejected", "The TE rejection was saved.");
      }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-black text-white">Submission review queue</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
            Pending TE submissions from the backend queue.
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_24rem]">
        <div className="space-y-3">
          {isLoading ? (
            <div className="rounded-lg border border-white/10 bg-slate-900/75 p-5 text-sm text-slate-300">
              Loading queue from backend...
            </div>
          ) : null}

          {!isLoading && queue.length === 0 ? (
            <div className="rounded-lg border border-white/10 bg-slate-900/75 p-5 text-sm text-slate-300">
              No submissions waiting for TE review.
            </div>
          ) : null}
        ...
        <aside className="rounded-lg border border-white/10 bg-slate-900/75 p-5">
          <h3 className="text-lg font-bold text-white">Review actions</h3>
          {selected ? (
            <div className="mt-4 space-y-4">
              ...
              <textarea
                ...
                placeholder="Recommendation, revision reason, or rejection reason"
              />
              ...
              <ActionButton
                icon={<Send size={16} />}
                label="Recommend to EB"
                loading={runningAction === "recommend"}
            ...
            <p className="mt-4 text-sm text-slate-400">Open a submission to review it.</p>
          )}
        </aside>
      </section>
    </div>
