...
export default function TaskDetailPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      setTask(await mangaErpApi.getPageTask(id));
    } catch (error) {
      toast.error(
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="border border-white/10 bg-slate-900 p-6">
        <ClipboardCheck size={22} className="text-cyan-200" />
        <h1 className="mt-4 text-2xl font-black text-white">
          {task?.chapterTitle ?? "Task detail"}{" "}
          {task ? `· Page ${task.pageNumber}` : ""}
        </h1>
        {loading ? (
          <p className="mt-4 text-sm text-slate-400">Loading task...</p>
        ) : task ? (
          <dl className="mt-6 grid gap-4 md:grid-cols-2 text-sm">
            ...
            <Info
              ...
              value={task.assignedAssistantId ?? "-"}
            />
            <Info
              label="Description"
          ...
          <p className="mt-4 text-sm text-slate-500">Task not found.</p>
        )}
      </section>
    </div>
  );
