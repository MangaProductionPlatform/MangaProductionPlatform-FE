...
export default function QaSubmissionPage() {
  ...
  const readiness = useMemo(
    () => evaluateChapterQaReadiness(selectedChapter, tasks),
    [selectedChapter, tasks],
  );
  const isReady = readiness.isReady;
  const progress = tasks.length
    ? Math.round((readiness.approvedTaskCount / tasks.length) * 100)
    : 0;
  ...
  const loadTasks = async (id: string) => {
    ...
    try {
      ...
      try {
        setQaPins(await mangaErpApi.getQaPins(id));
      } catch (error) {
        setQaPins([]);
        toast.error(
          "Could not load Editorial QA feedback",
  ...
  useEffect(() => {
    void mangaErpApi
      ...
      .then((items) => {
        ...
        if (firstSeriesId) {
          void changeSeries(firstSeriesId);
        } else {
          setIsLoading(false);
        }
      })
  ...
  const submit = async () => {
    if (!chapterId || !isReady) {
      toast.error(
        ...
        "Every created page task must be approved before QA submission.",
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        "Chapter submitted to QA",
        "The Tantou Editor can now review the completed chapter.",
      );
      setChapters((items) =>
        items.map((item) =>
          item.id === chapterId ? { ...item, status: "InQA" } : item,
        ),
      );
    } catch (error) {
      toast.error(
        "QA submission failed",
        error instanceof Error ? error.message : "Unknown error",
      );
    } finally {
  ...
  return (
    <div className="qa-submission-page space-y-6">
      <header>
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-cyan-200">
          Mangaka · MF2/MF3 handoff
        </p>
        <h1 className="mt-2 text-3xl font-black text-white">
          QA Submission & Corrections
        </h1>
        <p className="mt-2 text-sm text-slate-400">
          Submit a completed chapter, receive the Editor feedback batch, and
          replace corrected layers on their existing page tasks.
        </p>
      </header>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="text-sm text-slate-400">
            ...
            <select
              className="input mt-2"
              value={chapterId}
              onChange={(event) => void loadTasks(event.target.value)}
            >
              <option value="">Select chapter</option>
              {chapters.map((item) => (
                <option key={item.id} value={item.id}>
                  Ch. {item.chapterNumber} — {item.title} ({item.status})
                </option>
              ))}
            </select>
          </label>
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              {isReady ? (
                ...
                <CircleDashed className="text-amber-300" />
              )}
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {readiness.approvedTaskCount} of {tasks.length} page tasks
                  approved
                </p>
              </div>
            </div>
            <FileCheck2
              className={isReady ? "text-emerald-300" : "text-slate-600"}
              size={30}
            />
          </div>
          ...
          {!isLoading && tasks.length > 0 ? (
            <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {tasks.map((task) => (
                <div
                  key={task.id}
                  className="flex items-center justify-between gap-3 rounded-lg bg-slate-950/70 px-3 py-2 text-sm"
                >
          ...
          {!isLoading && tasks.length === 0 ? (
            <p className="mt-4 text-sm text-amber-200">
              This chapter has no created page tasks yet.
            </p>
          ) : null}

          {!isLoading && readiness.issues.length > 0 ? (
            <div className="mt-4 rounded-xl border border-amber-400/20 bg-amber-500/10 p-4">
              <p className="text-sm font-semibold text-amber-100">
                QA submission is blocked until readiness can be verified.
              </p>
      ...
      {qaPins.length > 0 ? (
        <section className="rounded-2xl border border-amber-300/20 bg-slate-900 p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-200">
            Editor feedback batch
          </p>
          <h2 className="mt-2 text-xl font-bold text-white">
            Correct the existing page tasks
          </h2>
          <p className="mt-2 text-sm text-slate-400">
            The official API has no QA reassignment or chapter-resubmission
            operation. The Mangaka or originally assigned Assistant uploads the
            corrected layer to the same page task; the Editor then rechecks and
            resolves each pin.
          </p>

          <div className="mt-5 space-y-3">
            {qaPins.map((pin) => {
              const task = tasks.find((item) => item.id === pin.pageTaskId);
              ...
              return (
                ...
                >
                  <p className="font-semibold text-white">
                    {pin.issueType ?? "Quality issue"} · Page{" "}
                    {task?.pageNumber ?? "—"}
                  </p>
                  ...
                  <p className="mt-2 break-all text-xs text-slate-500">
                    Page task: {pin.pageTaskId} · Status: {pin.status}
                  </p>
                </article>
              );
            })}
          ...
          <p className="mt-4 rounded-lg border border-dashed border-slate-700 p-3 text-xs text-slate-500">
            TODO: Add Assistant QA-reassignment UI only when the backend
            publishes an official reassignment endpoint.
          </p>
        </section>
      ) : null}
    </div>
