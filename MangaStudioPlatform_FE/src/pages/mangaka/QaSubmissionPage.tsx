...
export default function QaSubmissionPage() {
  ...
  return <div className="space-y-6">
    ...
    {qaPins.length > 0 ? <section className="rounded-2xl border border-amber-300/20 bg-slate-900 p-6"><p className="text-xs font-semibold uppercase tracking-[.2em] text-amber-200">Editor feedback batch</p><h2 className="mt-2 text-xl font-bold text-white">Correct the existing page tasks</h2><p className="mt-2 text-sm text-slate-400">Upload each corrected layer to its existing page task. The Editor will recheck the result and resolve the reported issue.</p><div className="mt-5 space-y-3">{qaPins.map((pin) => { const task = tasks.find((item) => item.id === pin.pageTaskId); return <article key={pin.id} className="rounded-xl border border-white/10 bg-slate-950 p-4"><p className="font-semibold text-white">{pin.issueType ?? "Quality issue"} · Page {task?.pageNumber ?? "—"}</p><p className="mt-2 text-sm text-slate-300">{pin.noteMessage}</p><p className="mt-2 break-all text-xs text-slate-500">Page task: {pin.pageTaskId} · Status: {pin.status}</p></article>; })}</div></section> : null}
  </div>;
}
