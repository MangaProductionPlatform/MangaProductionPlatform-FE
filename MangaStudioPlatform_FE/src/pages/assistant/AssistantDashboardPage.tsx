...
export default function AssistantDashboardPage() {
  ...
  const respond = async (id: string, response: "accept" | "decline") => {
    setBusy(id);
    try {
      await mangaErpApi.respondToInvitation(id, response);
      toast.success(
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {items.map((x) => (
          ...
          >
            <h3 className="font-bold text-white">Series {x.seriesId}</h3>
            <p className="mt-2 text-sm text-slate-400">
              {x.message || "You were invited to join this studio."}
            </p>
            <div className="mt-4 flex gap-2">
              <button
                disabled={busy === x.invitationId}
                className="rounded-lg bg-cyan-300 px-4 py-2 font-bold text-slate-950"
                onClick={() => void respond(x.invitationId, "accept")}
              >
                Accept
              </button>
              <button
                ...
                onClick={() => void respond(x.invitationId, "decline")}
              >
                Decline
              </button>
            </div>
        ...
        {!items.length ? (
          <section className="rounded-2xl border border-white/10 bg-slate-900/80 p-6">
            <div className="flex max-w-sm gap-4">
              ...
              <div>
                <h3 className="font-bold text-white">No pending invitations</h3>
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  When a Mangaka invites you to a studio, the invitation will
                  appear here.
                </p>
                <button
                  type="button"
                  onClick={() => void load()}
                  className="mt-4 inline-flex items-center gap-2 rounded-lg border border-cyan-300/30 bg-cyan-300/10 px-3 py-2 text-sm font-semibold text-cyan-100"
                >
                  <RefreshCw size={15} />
                  Refresh
                </button>
